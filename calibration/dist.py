...
def one_wasserstein_distance(
    alpha, mu, sigma, weight, mean, variance, start=-5, end=5, steps=100
):
    ...
    a = torch.linspace(start, end, steps, device=alpha.device).unsqueeze(-1)
    return torch.sum(
        torch.abs(
            cdf_gaussian_mixture(a, weight, mean, variance)
            - cdf_gaussian_mixture(a, alpha, mu, sigma)
        ),
        dim=-1,
    )
...
def cdf_gaussian_mixture(x, weight, mean, variance):
    return torch.sum(weight * cdf_gaussian(x, mean, variance), dim=-1)
